...
for majorkey, subdict in output_json.items():
	if majorkey == 'DBPRIINFO':
		print("Majorkey is",majorkey)
		print("Subdict is ",subdict)
		for db_name_details in subdict:
			print("db name and its details are ",db_name_details)
			for dbname, dbdetails in db_name_details.items():
				print("dbname is ",dbname)
				print("PRI DB PASS",dbdetails[0])
				passwd = dbdetails[0]
				print("DR DB NAME",dbdetails[1])
				drname = dbdetails[1]
				print("DR DB PASS",dbdetails[2])
				drpwd = dbdetails[2]
				connection = cx_Oracle.connect(uid + "/" + passwd + "@" + host + ":" + port + "/" + dbname)
				cursor = connection.cursor()
